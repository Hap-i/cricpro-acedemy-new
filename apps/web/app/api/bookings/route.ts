...
const notConfigured = () =>
  NextResponse.json({ success: false, error: 'Database not configured. Add Supabase credentials to .env.local.' }, { status: 503 });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const {
      resourceId, serviceType, bookingDate, startAt, endAt,
      customerName, customerEmail, customerPhone, playerCount, notes
    } = body;

    if (!serviceType || !bookingDate || !startAt || !endAt || !customerName || !customerEmail) {
    ...
    if (laneBasedServices.includes(serviceType) && !resourceId) {
      ...
      if (!allLanes || allLanes.length === 0) {
        return NextResponse.json({ success: false, error: 'No lanes available' }, { status: 409 });
      }
      ...
      const { data: laneBookings } = await supabaseAdmin
        .from('bookings')
        .select('resource_id')
        .in('resource_id', allLanes.map(l => l.id))
        .in('status', ['confirmed', 'pending_payment'])
        .lt('start_at', endAt)
        .gt('end_at', startAt);

      const bookedLaneIds = new Set((laneBookings || []).map(b => b.resource_id));
      const availableLane = allLanes.find(l => !bookedLaneIds.has(l.id));

      if (!availableLane) {
        return NextResponse.json({ success: false, error: 'All lanes are fully booked for this time slot' }, { status: 409 });
      }
    ...
    if (blockedSlots && blockedSlots.length > 0) {
      return NextResponse.json({ success: false, error: 'Time slot is blocked' }, { status: 409 });
    }
    ...
    if (paymentsEnabled) {
      try {
        const serviceLabel = serviceType.replace(/_/g, ' ').replace(/\b\w/g, (c: string) => c.toUpperCase());
        ...
        await supabaseAdmin.from('bookings').update({ stripe_session_id: sessionId }).eq('id', data.id);
        return NextResponse.json({ success: true, booking: data, paymentUrl: url, expiresAt: booking.expires_at });
      } catch (stripeError) {
        console.error('Stripe session creation failed:', stripeError);
        // Fall through and confirm without payment
      }
    }

    // Confirmed immediately (payments disabled or Stripe error)
    if (status === 'confirmed') {
      sendBookingConfirmation({ ...data, customer_name: customerName, customer_email: customerEmail }).catch(console.error);
      sendAdminBookingNotification({ ...data, customer_name: customerName, customer_email: customerEmail }).catch(console.error);
    }

    return NextResponse.json({ success: true, booking: data, expiresAt: booking.expires_at });
  } catch (error: any) {
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    if (!['pending_payment', 'pending', 'confirmed'].includes(existingBooking.status)) {
